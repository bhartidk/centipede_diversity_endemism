## 22-May-20 Added functions to calculate species richness, weighted endemism and phylogenetic diversity each as a separate standalone function based on https://github.com/DanRosauer/phylospatial/blob/master/LineageModels/phylogenetic%20endemism.r (this parent script is also saved in the R scripts file)

calc_SR <- function(tree, sites_x_tips, presence=c("presence","abundance","probability")){
SR.vec <- apply(sites_x_tips, MARGIN = 1, FUN = sum)
return(SR.vec)
}

calc_WE <- function(tree, sites_x_tips, presence=c("presence","abundance","probability")){
...
calc_PD <- function(tree, sites_x_tips, presence=c("presence","abundance","probability")){
